...
#if defined(NDEBUG)
# error "Ninjacoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x66d149d1413932ca621d57c029c2c7f2aecd647539dad7926d3847067ee49758");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Ninjacoin: starting difficulty is 1 / 2^12
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Ninjacoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Ninjacoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Ninjacoin: 2.5 minutes
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x4b707f3903edb7f793662d881654110f2f8ffc8f478d058744926872e955e1fc");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1526356074;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084534426;

        if (fTestNet)
        {
            block.nTime    = 1526356020;
            block.nNonce   = 387357978;
        }
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // Ninjacoin: increase each by adding 2 to bitcoin's value.
...
void static NinjacoinMiner(CWallet *pwallet)
{
    printf("NinjacoinMiner started\n");
    ...
    try { loop {
        ...
        printf("Running NinjacoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("NinjacoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&NinjacoinMiner, pwallet));
}
